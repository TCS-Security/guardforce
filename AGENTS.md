...
- Supervisor mode: staff sign in with email + password, `staff_me()` picks the mode, screens live in `guard-app/app/supervisor/`, RPCs in `0012_supervisor_app.sql`, e2e in `dashboard/e2e/supervisor-app.spec.ts`. Field KYC capture needs `guards:kyc`, which the seeded Supervisor role does not hold; the buttons hide for such roles.
